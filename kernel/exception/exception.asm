...
DeclareFunction DumpRegisters()
	ReserveStackSpace cpu_dump, CPUState
	UpdateStackPtr

	mov_ts qword[ cpu_dump.rax ], rax
	mov_ts qword[ cpu_dump.rbx ], rbx
	mov_ts qword[ cpu_dump.rcx ], rcx	
	mov_ts qword[ cpu_dump.rdx ], rdx
	mov_ts qword[ cpu_dump.rsi ], rsi
	mov_ts qword[ cpu_dump.rdi ], rdi
	mov_ts qword[ cpu_dump.r8 ], r8
	mov_ts qword[ cpu_dump.r9 ], r9
	mov_ts qword[ cpu_dump.r10 ], r10
	mov_ts qword[ cpu_dump.r11 ], r11
	mov_ts qword[ cpu_dump.r12 ], r12
	mov_ts qword[ cpu_dump.r13 ], r13
	mov_ts qword[ cpu_dump.r14 ], r14
	mov_ts qword[ cpu_dump.r15 ], r15
	mov rax, qword[ rbp + 8 ]
	mov_ts qword[ cpu_dump.rip ], rax
	mov rax, rbp
	add rax, 16
	mov_ts qword[ cpu_dump.rsp ], rax
	mov rax, qword[ rbp ]
	mov_ts qword[ cpu_dump.rbp ], rax

	secure_call malloc( 10000, "Register dump string" )
	ReserveStackSpace OutputStr, KString, rax, 10000
	UpdateStackPtr


	secure_call OutputStr.append_str( {CONSOLE_CHANGEFG(COLOR_GREEN), "    Register dump v1.0!", 0x0A } )
	mov_ts rbx, cpu_dump.rax
	mov r15, StringSetup

	.DrawLoop:
		secure_call OutputStr.append_str( {" ",CONSOLE_CHANGEFG(COLOR_WHITE)})
		secure_call OutputStr.append_str( r15 )
		secure_call OutputStr.append_str( {" ",CONSOLE_CHANGEFG(COLOR_RED)})
		secure_call OutputStr.append_inth( qword[ rbx ] )
		test r15, 1
		jz .NoNewline
		secure_call OutputStr.nline()
	.NoNewline:
		add r15, 11
		add rbx, 8
		cmp r15, StringEnd
		js .DrawLoop
	secure_call ClearScreen()
	secure_call OutputStr.c_str()
	secure_call DrawString( rax )
	secure_call OutputStr.c_str()
	secure_call free( rax )

	mov_ts rax, qword[ cpu_dump.rax ]
	mov_ts rbx, qword[ cpu_dump.rbx ]
	mov_ts rcx, qword[ cpu_dump.rcx ]
	mov_ts rdx, qword[ cpu_dump.rdx ]
	mov_ts rsi, qword[ cpu_dump.rsi ]
	mov_ts rdi, qword[ cpu_dump.rdi ]
	mov_ts r8, qword[ cpu_dump.r8 ]
	mov_ts r9, qword[ cpu_dump.r9 ]
	mov_ts r10, qword[ cpu_dump.r10 ]
	mov_ts r11, qword[ cpu_dump.r11 ]
	mov_ts r12, qword[ cpu_dump.r12 ]
	mov_ts r13, qword[ cpu_dump.r13 ]
	mov_ts r14, qword[ cpu_dump.r14 ]
	mov_ts r15, qword[ cpu_dump.r15 ] 
EndFunction

align 4
StringSetup	db '    rax = ', 0
	   	db ' || rbx = ', 0
		db '    rcx = ', 0
		db ' || rdx = ', 0
		db '    rsi = ', 0
