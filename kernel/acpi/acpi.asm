...
;Reserve some space for a string, it will print important informations about the whereabouts
	...
	.StartParsingDescTable:
		...
		;Check if it is a acpi 1.0 table or a acpi 2.0+ table
			...
			secure_call MapVirtToPhys( rsi, rsi, 0x1000, PAGE_READ|PAGE_CACHE_TYPE_WT)

			pop rsi					;Restore rsi
			cmp dword[ rsi ], MADT_Signature	;Is the table an Multiple APIC Description Table?
			jnz .check_for_hpet			;If not check if it is the HPET Table

			;It is an Multiple APIC Description Table, supply it to the APIC Module
			secure_call SupplyACPIApicTable( rsi )

			jmp .selectNextTable

			.check_for_hpet:
				cmp dword[ rsi ], HPET_Signature	;Is it the HPET Table?
				jnz .selectNextTable

				secure_call SupplyHPETTable( rsi )
